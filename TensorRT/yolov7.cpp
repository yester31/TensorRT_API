...
static const int OUTPUT_SIZE = 1 * 32 * 544 * 640;
//static const int OUTPUT_SIZE = 1 * MAX_OUTPUT_BBOX_COUNT * 85;
...
void createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt, char* engineFileName)
{
    ...
    std::map<std::string, Weights> weightMap = loadWeights("../yolov7_py/yolov7.wts");
    ...
    IConvolutionLayer* conv_0 = network->addConvolutionNd(*prep, 32, DimsHW{ 3, 3 }, weightMap["model.0.conv.weight"], weightMap["model.0.conv.bias"]);
    conv_0->setStrideNd(DimsHW{ 1, 1 });
    conv_0->setPaddingNd(DimsHW{ 1, 1 });
    ITensor* silu_0 = addSilu(network, conv_0->getOutput(0));

    ITensor* final = silu_0;
    // detect
    show_dims(final);
    final->setName(OUTPUT_BLOB_NAME);
    network->markOutput(*final);
...
int main()
{
    ...
    char engineFileName[] = "yolov7";       // model name
